...
const STORAGE_KEY = "metodo-epc-demo-referencias-e-concorrentes";
...
type ReferencesCompetitorsData = {
...
const initialData: ReferencesCompetitorsData = {
  items: [createEmptyItem()],
};

function normalizeItem(item: Partial<ReferenceCompetitorItem>) {
  const legacyLink =
  ...
  if (legacyLink && !channels.site) {
    channels.site = legacyLink;
  }

  const legacyAnalysisTitle =
    typeof (item as { analysisTitle?: string }).analysisTitle === "string"
      ? (item as { analysisTitle?: string }).analysisTitle || ""
      : "";

  const legacyAnalysisDescription =
    typeof (item as { analysisDescription?: string }).analysisDescription ===
    "string"
      ? (item as { analysisDescription?: string }).analysisDescription || ""
      : "";
  ...
  const analyses =
    Array.isArray(item.analyses) && item.analyses.length
      ...
      : [
          {
            title: legacyAnalysisTitle,
            description: legacyAnalysisDescription,
          },
        ];
          ...
          <div key={category}>
            ...
            <div className="space-y-1">
              {planningModules
                ...
                .map((module) => {
                  const isActive =
                    module.slug === "referencias-e-concorrentes";
...
export default function ReferenciasEConcorrentesPage() {
  const [data, setData] = useState<ReferencesCompetitorsData>(initialData);
      ...
      setData({
        items:
          Array.isArray(parsed.items) && parsed.items.length
            ? parsed.items.map((item: Partial<ReferenceCompetitorItem>) =>
                normalizeItem(item)
              )
            : initialData.items,
      });
          ...
          <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-slate-600">
                Defina referências, concorrentes e inspirações estratégicas para
                entender o mercado, comparar posicionamentos e construir uma
                direção original para o projeto.
              </p>
                      ...
                      <div className="grid gap-5">
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white p-5">
                          <div>
                            ...
                            <p className="mt-2 text-sm leading-6 text-slate-500">
                              Preencha apenas os canais que serão considerados na
                              análise. Eles aparecerão na apresentação como
                              botões clicáveis com ícone.
                            </p>
                          </div>
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white p-5">
                          <div>
                            ...
                            <p className="mt-2 text-sm leading-6 text-slate-500">
                              Registre os pontos observados nesta referência ou
                              concorrente, como posicionamento, pontos fortes,
                              diferenciais, riscos, oportunidades e aprendizados.
                            </p>
                          </div>
                              ...
                              >
                                <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                  <div>
                                    <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-400">
                                      Análise{" "}
                                      {String(analysisIndex + 1).padStart(
                                        2,
                                        "0"
                                      )}
                                    </p>
                                  ...
                                  <button
                                    type="button"
                                    onClick={() =>
                                      removeAnalysis(index, analysisIndex)
                                    }
                                    className="cursor-pointer rounded-full px-4 py-2 text-sm font-semibold text-red-500 transition hover:bg-red-50"
                                  >
                                    Excluir
